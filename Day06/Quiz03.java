...
public class Quiz03 {
    public static int countJava(String str)
    {
        ...
        {
            if(strArr2[i].equals("Java")) //equalsiIgnoreCase를 사용하면 대소문자 구분없이 가능.
            {
                count2++;
            }
        }
        return count2;
        //문자열이 :으로 구분되어 있기 떄문에 split메서드를 사용하여 문자들을 배열로 만들고 해당 배열이 "Java"와 동일한 문자라면 카운트가 1개 증가.
    }
    public static void main(String[] args) {
        ...
        int count = 0;
        String[] strArr = str.split("Java");
        for(int i=0; i< strArr.length; i++)
        {
            System.out.println(strArr[i]);
            count++;
        }
        //split 메서드 사용시 regex에 해당하는 Java를 기준으로 문자열이 나뉨. 즉, Java의 문자 개수는 총 배열의 개수의 -1개 만큼 존재함.

        System.out.println(count-1); //1번 방법
        System.out.println(countJava(str)); //2번 방법 메서드 구현
    }
}
